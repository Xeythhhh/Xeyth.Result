...
public class BindValueTask : TestBase
{
    [Fact]
    public async Task ShouldBind_WhenInitialResultIsSuccessful_AndBindingResultIsSuccessful() =>
        await Verify(
            async () => await Result.Ok().WithSuccess("Initial Result Success")
                .Bind(() => ValueTask.FromResult(Result.Ok().WithSuccess("Binding Result Success"))),
            Settings);
    ...
    public async Task ShouldBind_WhenInitialResultIsSuccessful_AndBindingResultIsFailure() =>
        await Verify(
            async () => await Result.Ok().WithSuccess("Initial Result Success")
                .Bind(() => ValueTask.FromResult(Result.Fail("Binding Result Error"))),
            Settings);
    ...
    public async Task ShouldBind_WhenInitialResultIsSuccessful_AndGenericBindingResultIsSuccessful() =>
        await Verify(
            async () => await Result.Ok().WithSuccess("Initial Result Success")
                .Bind(() => ValueTask.FromResult(Result.Ok(420).WithSuccess("Binding Result Success"))),
            Settings);
    ...
    public async Task ShouldThrowArgumentNullException_WhenBindActionIsNull() =>
        await ThrowsValueTask(() => Result.Ok().Bind((Func<ValueTask<Result>>)null!), Settings)
            .IgnoreStackTrace();

    [Fact]
    public async Task ShouldThrowArgumentNullException_WhenGenericBindActionIsNull() =>
        await ThrowsValueTask(() => Result.Ok().Bind((Func<ValueTask<Result<object>>>)null!), Settings)
            .IgnoreStackTrace();
}
