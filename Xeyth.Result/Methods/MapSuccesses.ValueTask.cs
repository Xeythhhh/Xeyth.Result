...
public partial class Result
{
    ...
    /// <code><see cref="SuccessParallelMappingThreshold"/> = 42;</code></example></summary>
    ...
    /// <para>If the <paramref name="processInParallel"/> flag of the method is supplied, this property is ignored.</para></remarks>
    public static int SuccessParallelMappingThreshold { get; set; } = 5;
    ...
    /// <returns>A <see cref="ValueTask"/> representing the operation, containing a new <see cref="Result"/> with mapped successes.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the <paramref name="successMapper"/> is <see langword="null"/>.</exception>
    public async ValueTask<Result> MapSuccesses<TSuccess>(Func<ISuccess, ValueTask<TSuccess>> successMapper, bool? processInParallel = null)
        where TSuccess : ISuccess
    {
        ArgumentNullException.ThrowIfNull(successMapper);

        processInParallel ??= Successes.Count > SuccessParallelMappingThreshold;
        ...
        async ValueTask<Result> MapSuccessesInParallel(Func<ISuccess, ValueTask<TSuccess>> successMapper)
        {
        ...
        async ValueTask<Result> MapSuccessesSequentially(Func<ISuccess, ValueTask<TSuccess>> successMapper)
        {
