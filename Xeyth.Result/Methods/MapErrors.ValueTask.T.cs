...
#pragma warning restore RCS1158 // Static member in generic type should use a type parameter
    ...
    /// <returns>A <see cref="ValueTask"/> representing the operation, containing a new <see cref="Result{TValue}"/> with mapped errors.</returns>
    /// <exception cref="ArgumentNullException">Thrown when the <paramref name="errorMapper"/> is <see langword="null"/>.</exception>
    public async ValueTask<Result<TValue>> MapErrors<TError>(Func<IError, ValueTask<TError>> errorMapper, bool? processInParallel = null)
        where TError : IError
    {
        ...
        async ValueTask<Result<TValue>> MapErrorsInParallel(Func<IError, ValueTask<TError>> errorMapper)
        {
        ...
        async ValueTask<Result<TValue>> MapErrorsSequentially(Func<IError, ValueTask<TError>> errorMapper)
        {
